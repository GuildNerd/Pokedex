...
export default function Display({ imgUrl, pokemonName, pokemonId, pokemonType, isPokemonShown, handleShowPokemon }: DisplayProps) {
    ...
    return (
        <div className="flex flex-col p-4 pb-0 text-center justify-center rounded-sm rounded-bl-2xl shadow-md bg-white shadow-gray-400">
            ...
            <div className="grid grid-cols-3 justify-center mt-2 w-60">
                ...
                {   isPokemonShownLocal ? <p>{ pokemonName.charAt(0).toUpperCase() + pokemonName.substring(1) }</p> : null    }
                {   isPokemonShownLocal ? <p>{ `#${pokemonId.toString().padStart(4, '0')}` }</p> : null    }
            </div>
        </div>
    )
}